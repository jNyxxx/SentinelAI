...
export default function IncidentDetail() {
  ...
  // Parse FULL report from text file (not database field)
  const reportSections = incident.fullReport
    ? parseReportSections(incident.fullReport)
    : null;
  ...
  function parseReportSections(reportText) {
    ...
    for (const line of lines) {
      ...
      // Skip formatting lines (dashes, decorative lines, headers)
      if (trimmed.match(/^[=\-]+$/) ||  // Lines with only = or -
          trimmed === 'INCIDENT REPORT' ||
          trimmed === 'FULL REPORT' ||
          trimmed === 'END OF REPORT') {
        continue;  // Skip these lines
      }
      
      // Match section headers more flexibly
      const isSectionHeader = trimmed.match(/^[A-Z][a-zA-Z\s]+:/) || 
                              trimmed.match(/^[A-Z]+:$/) ||
    ...
    <>
      ...
      <main className="flex-1 overflow-y-auto p-6 bg-grid space-y-6">
        ...
        <div className="grid grid-cols-12 gap-6">
          {/* Left Column - Evidence Photos + Metadata */}
          <div className={((incident.riskLevel === 'High' || incident.riskLevel === 'Medium') && photos.length > 0) ? 'col-span-4' : 'hidden'}>
          ...
          <div className={((incident.riskLevel === 'High' || incident.riskLevel === 'Medium') && photos.length > 0) ? 'col-span-8' : 'col-span-12'}>
            <div className="space-y-4">
              ...
              {/* Main Report Content */}
              <div className="bg-bg-primary/50 rounded-lg p-6 border border-bg-border/50 mb-6">
                ...
                {reportSections && reportSections.length > 0 ? (
                  <div className="space-y-6">
                    {reportSections.map((section, idx) => (
                      <div key={idx} className="space-y-3">
                        <h4 className="text-base font-semibold text-text-primary border-b border-bg-border/50 pb-2">
                          {section.title}
                        </h4>
                        <div className="space-y-3">
                          {section.content.map((paragraph, pIdx) => (
                            <p
                              key={pIdx}
                              className="text-text-secondary text-sm leading-relaxed font-sans whitespace-pre-wrap break-words"
                              style={{ overflow: 'visible', textOverflow: 'visible', wordBreak: 'break-word' }}
                            >
                              {paragraph}
                            </p>
                          ))}
                  ...
                  <div className="prose prose-invert prose-sm max-w-none">
                    <p className="text-text-secondary text-sm leading-relaxed whitespace-pre-wrap break-words font-sans" style={{ overflow: 'visible', textOverflow: 'visible' }}>
                      {incident.fullReport || 'No detailed report available.'}
                    </p>
                  </div>
                )}
              </div>

              {/* Analysis Details - Added spacing */}
              <div className="bg-bg-card border border-bg-border rounded-xl p-6 card-glow mt-6">
                <h3 className="text-sm font-semibold text-text-primary mb-4 flex items-center gap-2">
